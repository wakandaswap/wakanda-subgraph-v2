...
import { updatePairDayData, updateTokenDayData, updateWakandaDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  wakanda.totalLiquidityBNB = wakanda.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  wakanda.totalLiquidityBNB = wakanda.totalLiquidityBNB.plus(trackedLiquidityBNB);
  wakanda.totalLiquidityUSD = wakanda.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  wakanda.totalTransactions = wakanda.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let wakanda = WakandaFactory.load(FACTORY_ADDRESS);
  wakanda.totalVolumeUSD = wakanda.totalVolumeUSD.plus(trackedAmountUSD);
  wakanda.totalVolumeBNB = wakanda.totalVolumeBNB.plus(trackedAmountBNB);
  wakanda.untrackedVolumeUSD = wakanda.untrackedVolumeUSD.plus(derivedAmountUSD);
  wakanda.totalTransactions = wakanda.totalTransactions.plus(ONE_BI);
  ...
  let wakandaDayData = updateWakandaDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  wakandaDayData.dailyVolumeUSD = wakandaDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  wakandaDayData.dailyVolumeBNB = wakandaDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  wakandaDayData.dailyVolumeUntracked = wakandaDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  wakandaDayData.save();
